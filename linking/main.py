...
from dataset import build_hierarchy
...
from link import *
from naming import *
from test_cst_proto2_modify import *
...
def train_one_epoch(net, args, epoch, known_class_dataloader, new_class_dataloader, known_class_dataset,
    ...
    with tqdm(total=total_b, desc='training') as pbar:
        for iteration in range(total_b):
            ...
            # -------- 6) Hierarchy loss (Recursive Entailment) --------
            loss_h = torch.tensor(0., device=device)  # 初始化，防止报错
            if epoch > args.warmup:
                # 如果 k_batch 在步骤(3)被释放了或者没运行(pretrain阶段)，则需要重新取
                if 'k_batch' in locals() and k_batch is not None:
                    h_batch = k_batch
                else:
                    h_batch, known_class_iter = endless_get_next_batch(
                        loaders=known_class_dataloader, iters=known_class_iter, batch_size=args.b_size
                    )
                # 提取特征 (需要梯度)
                h_feats = net.forward(h_batch, msg='feat')
                h_labels = h_batch[3].to(device)
                loss_h = net.module.forward_hierarchy(
                    h_feats,
                    h_labels,
                    args.hierarchy_relations,
                    args.label2taxoid
                )
                (0.1 * loss_h).backward()
                loss += 0.1 * loss_h
...
def test_one_epoch(net, args, epoch, new_class_dataloader):
    ...
    with torch.no_grad():
        ...
        with tqdm(total=len(new_class_dataloader), desc='testing') as pbar:
            ...
            a = ACC(ground_truth, label_pred)
            print(
                "acc:{}, B3_f1:{}, B3_prec:{}, B3_rec:{}, v_f1:{}, v_hom:{}, v_comp:{}, ARI:{}, NMI:{}".format(a, B3_f1,
                                                                                                               B3_prec,
                                                                                                               B3_rec,
                                                                                                               v_f1,
                                                                                                               v_hom,
...
def evaluate_hierarchy_performance(net, args, known_class_train_dataloader, known_class_test_dataloader, new_class_test_dataloader, node2taxoid, name_relations, hierarchy_relations, taxoid2node):
    print(">>> Evaluating Hierarchy Performance...")
    net.eval()
    # update_centers_u(net, args, new_class_test_dataloader)

    # 获取预测的聚类标签列表 (用于聚合)
    pred_labels = []
    for batch in new_class_test_dataloader:
        # batch[-1] 是 pseudo label (在 update_centers_u 后被填充)
        # 注意：dataset.py 的 __getitem__ 需要确保返回 pseudo
        pred_labels.append(batch[-1])
    ...
    info_new = get_test_info2(args, new_class_test_dataloader, u_idx2trigger, net)
    ...
    # 1. 计算新类原型 (Query)
    print("Computing cluster prototypes (Query)...")
    new_cluster_protos = get_cluster_prototypes_with_aggregator(
        net, args, new_class_test_dataloader, None
    )

    print("Computing GOLDEN cluster prototypes (Oracle)...")
    golden_cluster_protos = get_golden_prototypes_with_aggregator(
        net, args, new_class_test_dataloader
    )

    # 2. 动态构建 Taxonomy 树向量 (Target) <-- 新增步骤
    print("Building dynamic taxonomy tree (Target)...")
    # 注意：需要把 hierarchy_relations 传进去
    dynamic_taxo_embeddings = build_dynamic_taxonomy_vectors(
        args, net, known_class_train_dataloader,  # 使用 Test 里的 Known Class 数据，或者 Train 里的
        l_trigger2idx, hierarchy_relations, taxoid2node
    )



    # 3. 执行链接
    print("Linking with Recursive Logic...")
    # 修改 link_inference_with_scorer 让它接受 dynamic_taxo_embeddings
    # 而不是在内部调用 net.taxonomy_embeddings

    # 这里你需要微调 link_inference_with_scorer 的代码，
    # 把 taxo_mem(child_ids_tensor) 改成从 dynamic_taxo_embeddings 字典里查
    res_new_logic = link_inference_with_scorer_dynamic(
        args, net, new_cluster_protos, info_new,
        dynamic_taxo_embeddings,  # 传入动态字典
        node2taxoid, name_relations
    )
    res_gold_logic = link_inference_with_scorer_dynamic(
        args, net, golden_cluster_protos, info_gold,
        dynamic_taxo_embeddings,  # 传入动态字典
        node2taxoid, name_relations
    )
    gold_hierarchy_cluster_list = get_glod_hierarchy_cluster_list(info, new_class_test_dataloader,
                                                                  u_trigger2idx)
    root_name = "event_type"
    print_gold_hierarchy_list(gold_hierarchy_cluster_list, root_name=root_name)
    # 打印基于预测聚类的路径
    print("res_new_logic", res_new_logic)
    print_taxonomy_paths(res_new_logic, root_name=root_name,
                         title="Predicted Logic Paths (based on Clustering) Aggregator")

    print("res_gold_logic", res_gold_logic)
    print_taxonomy_paths(res_gold_logic, root_name=root_name, title="Gold Logic Paths (based on GT Label) Aggregator")
    print("res", res)
    print_taxonomy_paths(res, root_name=root_name, title="Predicted Logic Paths (based on Clustering) Similarity")

    print("res_gold", res_gold)
    print_taxonomy_paths(res_gold, root_name=root_name, title="Gold Logic Paths (based on GT Label) Similarity")
    test_data_num = len(new_class_test_dataloader.dataset)

    predicted_cluster_list = get_predict_cluster_list(res_new_logic)
    predicted_cluster_list_similarity = get_predict_cluster_list(res)
    gold_cluster_list = get_predict_cluster_list(res_gold_logic)
    gold_cluster_list_similarity = get_predict_cluster_list(res_gold)
    ...
    print("predicting result similarity")
    evaluation = HierarchyClusterEvaluation(gold_hierarchy_cluster_list,
                                            predicted_cluster_list_similarity,
                                            test_data_num)
    eval_info = evaluation.printEvaluation()
    ...
    print("\n\n\n")
    print("gold result similarity")
    evaluation_gold = HierarchyClusterEvaluation(gold_hierarchy_cluster_list,
                                                 gold_cluster_list_similarity,
                                                 test_data_num)
    eval_info_gold = evaluation_gold.printEvaluation()
    print("\n\n\n")

def freeze_backbone_for_hierarchy(args, net):
    """
    冻结 BERT 和 聚类相关的 Projection Heads，
    只解冻 Aggregator, Entailment Scorer 和 Taxonomy Embeddings
    """
    # 1. 先冻结所有参数
    ...
    if hasattr(net, 'module'):
        ...
        model = net
    print(f"DEBUG: Unwrapped model type: {type(model)}")
    ...
    # 解冻 Scorer
    for param in model.entailment_scorer.parameters():
        param.requires_grad = True

    # 解冻 Taxonomy Embeddings (通常也需要微调)
    if not args.freeze_taxonomy_embeddings:
        model.taxonomy_embeddings.weight.requires_grad = True

    print(">>> Backbone and Clustering heads frozen. Only Hierarchy modules are trainable.")
    return net


def train_hierarchy_only_epoch(net, args, epoch, known_class_dataloader, optimizer):
    """
    只计算 forward_hierarchy 的 Loss
    """
    net.train()
    device = torch.device("cuda" if args.cuda else "cpu")
    epoch_hier_loss = 0

    # 只使用 known_class 数据来训练层级关系
    # 因为 unknown class 没有层级标签
    with tqdm(total=len(known_class_dataloader), desc=f'Training Hierarchy Epoch {epoch}') as pbar:
        for batch in known_class_dataloader:
            optimizer.zero_grad()

            # 1. 获取特征 (no_grad 或者是 eval 模式下的 forward，但因为前面 freeze 了参数，这里即使有 grad 也没事)
            # 建议：虽然 freeze 了参数，但为了节省显存，可以只对 hierarchy 部分求导
            # 但由于 aggregator 需要 feature 的梯度来更新，所以 feature 必须 detach 吗？
            # 不，aggregator 在 feature 之上，feature 不需要更新，所以 feature 视为常量输入

            # batch: input_ids ...
            # 注意：forward(msg='feat') 会过 BERT。因为 BERT freeze 了，所以单纯作为特征提取器
            feats = net.forward(batch, msg='feat')

            labels = batch[3].to(device)  # label ID

            # 2. 计算 Hierarchy Loss
            # 注意：这里调用 net.module.forward_hierarchy
            loss_h = net.module.forward_hierarchy(
                feats,
                labels,
                args.hierarchy_relations,
            ...
            epoch_hier_loss += loss_h.item()
            pbar.update(1)
            pbar.set_postfix({"hier_loss": epoch_hier_loss / (pbar.n)})

    return epoch_hier_loss / len(known_class_dataloader)
...
def main(args):
    ...
    num_taxo_nodes, label2taxoid, hierarchy_relations, init_embeds, node2taxoid, taxoid2node = build_hierarchy(
        args,
        l_trigger2idx,
        tokenizer,
        pretrained_model,
    )
    print("taxoid2node",taxoid2node)
    print("hierarchy_relations",hierarchy_relations)
    args.label2taxoid = label2taxoid
    args.taxoid2node = taxoid2node
    args.node2taxoid = node2taxoid
    # 保存到 args
    args.num_taxo_nodes = num_taxo_nodes
    args.hierarchy_relations = hierarchy_relations
    args.label2taxoid = label2taxoid
    ...
    if args.dataset == "ace":
        ...
        new_class_test_examples = PBertDataset.preprocess(args.root + args.test_class_filename, UNLABEL_TRIGGERS_ACE,
                                                          u_trigger2idx)
        structure_consts = ace_structure
    elif args.dataset == 'ere':
        ...
        new_class_test_examples = PBertDataset.preprocess_ere(args.root + args.test_class_filename,
                                                              UNLABEL_TRIGGERS_ERE, u_trigger2idx)
        structure_consts = ere_structure
    elif args.dataset == 'maven':
        ...
        new_class_test_examples = PBertDataset.preprocess_maven(args.root + args.test_class_filename,
                                                                UNLABEL_TRIGGERS_MAVEN, u_trigger2idx)
        structure_consts = maven_structure
    ...
    known_class_test_dataloader = DataLoader(BertDataset(args, known_class_test_examples, tokenizer),
                                             batch_size=args.b_size, shuffle=False, num_workers=0,
    ...
    taxo_structure = net.taxo_structure
    aggregator = net.aggregator
    entailment_scorer = net.entailment_scorer
    num_taxo_nodes = net.num_taxo_nodes
    taxonomy_embeddings = net.taxonomy_embeddings
    taxo_structure = net.taxo_structure
    ...
    if args.load_checkpoint_path:
        ...
        # 【关键修改点】
        # 如果加载进来的 checkpoint 已经是 DataParallel，我们要把它剥离出来
        # 这样后面 args.cuda 的时候才会正确地只包一层，而不是包两层
        if isinstance(checkpoint, nn.DataParallel):
            net = checkpoint.module  # <--- 取出 .module
        elif isinstance(checkpoint, dict) and 'state_dict' in checkpoint:
            # 兼容加载 state_dict 的情况 (更稳健)
            net.load_state_dict(checkpoint['state_dict'])
        else:
            net = checkpoint
    if not hasattr(net, 'global_dynamic_cache'):
        print(">>> Warning: 'global_dynamic_cache' not found in checkpoint. Initializing it manually.")
        net.global_dynamic_cache = {}
    if not hasattr(net, 'taxo_structure'):
        print(">>> Warning: 'taxo_structure' not found in checkpoint. Initializing it manually.")
        net.taxo_structure = taxo_structure
    if not hasattr(net, 'aggregator'):
        print(">>> Warning: 'aggregator' not found in checkpoint. Initializing it manually.")
        net.aggregator = aggregator
    if not hasattr(net, 'entailment_scorer'):
        print(">>> Warning: 'entailment_scorer' not found in checkpoint. Initializing it manually.")
        net.entailment_scorer = entailment_scorer
    if not hasattr(net, 'num_taxo_nodes'):
        print(">>> Warning: 'num_taxo_nodes' not found in checkpoint. Initializing it manually.")
        net.num_taxo_nodes = num_taxo_nodes
    if not hasattr(net, 'taxonomy_embeddings'):
        print(">>> Warning: 'taxonomy_embeddings' not found in checkpoint. Initializing it manually.")
        net.taxonomy_embeddings = taxonomy_embeddings
    if not hasattr(net, 'taxo_structure'):
        print(">>> Warning: 'taxo_structure' not found in checkpoint. Initializing it manually.")
        net.taxo_structure = taxo_structure
    net.fuse_concept = args.fuse_concept
    net.pool_strategy = args.pool_strategy
    net.score_neg_loss = args.score_neg_loss
    net.score_pos_loss = args.score_pos_loss
    net.aggregator_loss = args.aggregator_loss
    net.noise_ratio = args.noise_ratio


    # === 新增：冻结逻辑 ===
    if args.train_hierarchy_only:
        net = freeze_backbone_for_hierarchy(args, net)
        # 重新定义 Optimizer，只包含 requires_grad=True 的参数
        # 这一步很关键，否则 Optimizer 会去更新那些不需要梯度的参数（虽然梯度为0，但浪费内存）
    ...
    # num_taxo_nodes, label2taxoid, hierarchy_relations, _, node2taxoid, taxoid2node = build_hierarchy(
    #     args,
    #     l_trigger2idx,
    #     tokenizer,
    #     pretrained_model,
    ...
    # 临时构建一个 Name based relations 用于 Top-Down 搜索
    name_relations = {}
    ...
    def parse_structure(struct_list):
        for item in struct_list:
            p = item[0][0]
            c = item[1][0]
            if p not in name_relations: name_relations[p] = []
            name_relations[p].append(c)

    parse_structure(structure_consts)
    update_centers_u(net, args, new_class_test_dataloader)
    update_centers_u(net, args, new_class_train_dataloader)
    for epoch in range(1, args.epochs + 1):
        print("\n-------EPOCH {}-------".format(epoch))
        if args.train_hierarchy_only:
            print("Evaluting Hierarchy...")
            # evaluate_hierarchy_performance(net, args, known_class_train_dataloader, known_class_test_dataloader, new_class_test_dataloader, node2taxoid, name_relations, hierarchy_relations, taxoid2node)
            # 1. 仅训练 Hierarchy
            train_hierarchy_only_epoch(net, args, epoch, known_class_train_dataloader, optimizer)

            # 2. 评估 Hierarchy (Link)
            # 为了在训练过程中监控，我们需要调用评估脚本
            # 这里调用我们刚刚写的评估逻辑，或者是你 test_cst_proto2.py 里的逻辑
            if epoch > 1 and epoch % args.save_freq == 0:
                print("Evaluting Hierarchy...")
            ...
            if result > best_result:
                ...
                torch.save(net, f"{args.save}model_{args.dataset}_{args.save_comment}_best.pt")
                # link
            # === 新增代码：每 n 个 epoch 保存一次 checkpoint ===
            if args.save_freq > 0 and epoch % args.save_freq == 0:
                # 文件名带上 epoch 数，防止覆盖
                checkpoint_path = f"{args.save}model_{args.dataset}_{args.save_comment}_epoch_{epoch}.pt"
                print(f"Saving periodic checkpoint to: {checkpoint_path}")
...
if __name__ == '__main__':
    ...
    parser.add_argument("--fuse_concept", action="store_true", default=False)
    ...
    parser.add_argument("--train_hierarchy_only", action="store_true",
                        help="Freeze BERT and clustering, only train Aggregator and Scorer")
    parser.add_argument("--freeze_taxonomy_embeddings", action="store_true",
                        help="freeze_taxonomy_embeddings")

    parser.add_argument("--load_checkpoint_path", type=str, default=None,
                        help="Path to the pre-trained clustering model")
    parser.add_argument("--score_neg_loss", type=float, default=2.0)
    parser.add_argument("--score_pos_loss", type=float, default=2.0)
    parser.add_argument("--aggregator_loss", type=float, default=0.1)
    ...
    if not os.path.exists(args.save):
        ...
        print(f"Created directory: {args.save}")
