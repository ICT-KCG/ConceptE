...
def get_glod_hierarchy_cluster_list(info, dataloader, dict):
    ...
    # for iteration, (input_ids, input_mask, valid_mask, label, pos_span, mask_span, arg_spans, trg_concept, arg_concepts, idx) in enumerate(
    #             dataloader):  # (batch_size, seq_len), (batch_size)
    for iteration, data in enumerate(dataloader):  # (batch_size, seq_len), (batch_size)
        label = data[3]
        idx = data[-2]
...
def get_test_info2(args, dataloader, dict, net):
    info = [{"name":None, "emb": None, "instance":[],"vec":[], "name_LLM":None} for i in dict]
    net.eval()
    device=torch.device("cuda" if args.cuda else "cpu")
